...
//        stream.peek(System.out::println);
        //测试了上面给出的代码你会发现，***压根不会按照逻辑跑***
        stream.peek(System.out::println).collect(Collectors.toList()).forEach(System.out::println);
        //测试了下面给出的代码你会发现，***同样压根不会按照逻辑跑***
     /*起始生成阶段。
    中间操作会逐一获取元素并进行处理。 可有可无。所有中间操作都是惰性的，因此，流在管道中流动之前，任何操作都不会产生任何影响。
    终端操作。通常分为 最终的消费 （foreach 之类的）和 归纳 （collect）两类。
        ***还有重要的一点就是终端操作启动了流在管道中的流动。*/
//        Stream<String> reStream = stream.peek(new Consumer<String>() {
//            @Override
//            public void accept(String string) {
//                System.out.println(string);
//            }
//        });
//        new JDK8_features().testLambda();
        new JDK8_features().testStream();
    }
    /**
     * 1.Lambda表达式
     */
